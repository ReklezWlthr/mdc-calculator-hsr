...
export const MiniRelicBlock = observer(({ aId, index }: { aId: string; index: number }) => {
  ...
  const onOpenSwapModal = useCallback(() => {
    modalStore.openModal(<ArtifactListModal index={index} type={relic?.type} />)
  }, [index, aId])

  return (
    <div
      className="px-3 py-1.5 rounded-lg bg-primary-dark cursor-pointer hover:scale-[97%] duration-200 hover:ring-2 ring-primary-light"
