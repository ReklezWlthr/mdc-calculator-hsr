...
export const LCTeamConditionals: IWeaponContent[] = [
  ...
  },
  {
    ...
    default: true,
    debuff: true,
    chance: { base: 1, growth: 0.15, fixed: false },
    duration: 2,
    id: '23023_1',
    excludeSummon: true,
    scaling: (base, form, r, { debuffs, own }) => {
  ...
  },
  {
    ...
    default: true,
    debuff: true,
    chance: { base: 0.6, fixed: false, growth: 0.1 },
    id: '21015',
    excludeSummon: true,
    scaling: (base, form, r, { debuffs, own }) => {
      if (form['21015']) {
  ...
  },
  {
    ...
    default: true,
    debuff: true,
    chance: { base: 1, growth: 0.05, fixed: false },
    duration: 2,
    id: '21016',
    ...
    scaling: (base, form, r, { own, debuffs }) => {
      if (form['23052_a']) {
        base.VULNERABILITY.push({
          ...
          value: calcRefinement(0.1, 0.02, r) * (form['23052_b'] ? 1 + calcRefinement(0.6, 0.05, r) : 1),
        })
        if (base.SUMMON_STATS) {
          base.SUMMON_STATS.VULNERABILITY.push({
            name: `Blank`,
    ...
    scaling: (base, form, r) => {
      if (form['23052_b']) {
        base[Stats.CRIT_DMG].push({
          name: `Verse`,
          source: `This Love, Forever`,
          value: calcRefinement(0.16, 0.03, r) * (form['23052_a'] ? 1 + calcRefinement(0.6, 0.05, r) : 1),
        })
        if (base.SUMMON_STATS) {
          base.SUMMON_STATS[Stats.CRIT_DMG].push({
            name: `Verse`,
    ...
    scaling: (base, form, r, { index, owner }) => {
      if (index === owner) {
        base.ULT_DMG.push({
          ...
          value: _.min([(calcRefinement(0.2, 0.05, r) * base.MAX_ENERGY) / 100, calcRefinement(0.72, 0.18, r)]),
          base: _.min([base.MAX_ENERGY, 360]),
          multiplier: calcRefinement(0.2, 0.05, r),
        })
      }
      if (form['23062'] && !checkBuffExist(base[Stats.CRIT_DMG], { name: `King's Entertainment` })) {
        base[Stats.CRIT_DMG].push({
